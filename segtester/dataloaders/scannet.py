...
class ScannetScene(Scene):
    ...
    def get_labelled_reproj_seg3d(self, depth_min=0.4, depth_max=10.0):
        ...
        inv_intr = np.invert(d_image_intrinsics)
        ...
        with ZipFile(self.projected_label_file, 'r') as labeled_arch, ZipFile(self.projected_instance_archive, 'r') as instance_arch:
            ...
            for depth_image, camera_to_world, i in self.get_depth_position_it():
                ...
                if lbl_filt_name not in n_list_lbl or lbl_seg_name not in n_list_inst:
                    continue
                lbl_img = imageio.read(labeled_arch.open(lbl_filt_name))
                lbl_seg_img = imageio.read(instance_arch.open(lbl_seg_name))

                scaled_d_img = depth_image/d_scale
                ...
                proj_points_cam[:, 4] = 1.0
                proj_points_world = camera_to_world*proj_points_cam
                pt_lbls = lbl_img[d_img_mask]
                pt_seg = lbl_seg_img[d_img_mask]
                ...
                s3d = Seg3D(
                    proj_points_world, pt_lbls, instance_masks, unique_segs[0], np.ones(len(pt_lbls))
                )
                yield s3d, inds[d_img_mask.ravel()]
    ...
    def get_seg_3d(self, label_map):
        ...
        for lbl in mask_labels:
            ...
            if lbl[-1] == 's' and lbl[:-1] in text_map:
                ...
                continue
            assert False, f"Could not file label: {text_map} in labelmap"
