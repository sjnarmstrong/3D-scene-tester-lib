        ...
        for scene in tqdm(scenes_sorted_by_id, desc="scene", total=len(est_dataset.scenes)):
            try:
                ...
                if curr_gt_scene_id != scene.id:
                    ...
                    for val in tqdm(curr_gt_scene.get_labelled_reproj_seg3d(), desc="loading gt segs"):
                        val[0].map_own_classes(gt_label_map)
                        val[1].map_own_classes(gt_label_map)
                        reproj_2d_segs.append(val)
                        # if len(reproj_2d_segs) == 3:
                        #     break
                        # visualise reprojected labels
                        # cmap = plt.get_cmap("hsv")
                        # pcd = o3d.geometry.PointCloud()
                        # pcd.points = o3d.utility.Vector3dVector(val[0].points)
                        # colors = np.ones_like(val[0].points)
                        # for i, inst_mask in enumerate(val[0].instance_masks):
                        #     colors[inst_mask] = cmap(i/len(val[0].instance_masks))[:3]
                        # pcd.colors = o3d.utility.Vector3dVector(colors)
                        # pcd2 = curr_gt_scene.get_pcd()
                ...
                for reproj_gt_seg, s2d_gt, seg_inds, img_nr in curr_gt_scene.get_labelled_reproj_seg3d():
                    ...
                    mapped_segs = np.zeros((mapped_est_seg.instance_masks.shape[0],) + s2d_gt.image_shape, dtype=np.int)
                    ...
                    np.savez_compressed(f"{save_path}/res_class_{img_nr}", **res_class)
                    np.savez_compressed(f"{save_path}/res_inst_{img_nr}", **res_inst)
                    np.savez_compressed(f"{save_path}/res_seg_{img_nr}", **res_seg)
                    np.savez_compressed(f"{save_path}/pvr_{img_nr}", precision=precision, recall=recall,
